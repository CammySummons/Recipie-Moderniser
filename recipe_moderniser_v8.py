...
Allowing fractional amounts for servings sizes.
...
get_instructions = input("Press <enter> to get instructions, or any other key + <enter>\n"
...
for recipe_line in full_recipe:
    ...
    if num_spaces > 1:  # Item has both unit and ingredient
        ...
        if amount[1]:  # ingredient_dictionary gives gms per 250 ml so conversion factor is 250
            amount_2 = general_converter(amount[0], ingredient, food_dictionary, 250)

            # If 'True' the ingredient is in the ingredient_dictionary, so converts it to grams
            if amount_2[1]:
